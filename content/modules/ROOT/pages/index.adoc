...
`{controller_url}`[AAP Web UI,window=_blank]
| Username: `{ansible_automation_platform_configuration_username}`
...
| Mattermost
| `{mattermost_url}`[Mattermost,window=_blank]
...
== Part 2: AI Driven Ansible for Network Automation

image::splunk-logo.png[splunk,150]

[IMPORTANT]
====
**Highly Recommended!** While Part 2 can be completed independently, we strongly encourage you to complete this section. Part 1 provides a **basic introduction** to AIOps concepts using a simple Apache service remediation. Part 2 takes these concepts further with a **more complex, real-world networking scenario** that demonstrates the true power of AI-driven automation at scale.
====

Part 2 synthesizes the 🧠 AI skills from the preceding modules into a practical network automation scenario. You will leverage AI to generate Ansible playbooks designed to troubleshoot **OSPF routing issues** on Cisco routers—a more sophisticated use case that showcases how AIOps can handle complex infrastructure challenges.

=== Why Network Automation?

This approach allows network experts to:

* **Codify deep troubleshooting knowledge** into automated, repeatable workflows
* **Lower the barrier to entry** for new engineers learning network operations
* **Validate and test automation playbooks** rapidly before production deployment

To simulate a realistic operational environment, this lab integrates **Splunk** and **Event-Driven Ansible**, creating a complete feedback loop to observe network events, decide on a course of action, and automatically act on OSPF-related issues.

=== Network Lab Topology

The network lab environment is composed of the following components:

* Splunk Enterprise
* Ansible Automation Controller
* Ansible Lightspeed AI API
...
a|
**Part 1: AIOps with Apache Remediation** (Modules 1-3)

A **basic introduction** to AIOps—learn the fundamentals by building a self-healing workflow for Apache service failures.

**Part 2: Network Automation** (Modules 4-5) ⭐ _Highly Recommended_

A **more complex scenario** applying AIOps to OSPF troubleshooting on Cisco routers. Demonstrates real-world network automation at scale.
|===
